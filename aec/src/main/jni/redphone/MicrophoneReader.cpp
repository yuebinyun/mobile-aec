...
#define TAG "AEC-MicrophoneReader"

#ifndef SL_ANDROID_RECORDING_PRESET_VOICE_COMMUNICATION
#define SL_ANDROID_RECORDING_PRESET_VOICE_COMMUNICATION ((SLuint32) 0x00000004)
#endif

MicrophoneReader::MicrophoneReader(int androidSdkVersion, AudioCodec &audioCodec,
                                   RtpAudioSender &rtpAudioSender, Clock &clock) :
...
void MicrophoneReader::recorderCallback(SLAndroidSimpleBufferQueueItf bufferQueue) {
    ...
    if (rawFile) {
        fwrite(inputBuffer, FRAME_SIZE * 2, 1, rawFile);
        fflush(rawFile);
    }

    // 编码，回声抑制，抗噪处理
    int encodedAudioLen = audioCodec.encode(inputBuffer, encodedAudio, sizeof(encodedAudio));
    encodedAudioLen += audioCodec.encode(inputBuffer + FRAME_SIZE, encodedAudio + encodedAudioLen,
                                         sizeof(encodedAudio) - encodedAudioLen);

    // 发送数据
    rtpAudioSender.sendRTP(clock.tick(2), encodedAudio, encodedAudioLen);
...
int MicrophoneReader::start(SLEngineItf *engineEnginePtr) {
    ...
    __android_log_print(ANDROID_LOG_WARN, TAG, "aec-write clean open...");
    ...
    if ((*recorderObject)->GetInterface(recorderObject, SL_IID_ANDROIDCONFIGURATION,
                                        &androidConfig) == SL_RESULT_SUCCESS) {
        SLint32 recordingPreset = SL_ANDROID_RECORDING_PRESET_GENERIC;

        if (androidSdkVersion >= 14) {
            __android_log_print(ANDROID_LOG_WARN, TAG,
                                "Using voice communication Microphone preset...");
    ...
    if ((*recorderObject)->GetInterface(recorderObject, SL_IID_RECORD, &recorderRecord) !=
        SL_RESULT_SUCCESS) {
        return -1;
    }

    if ((*recorderObject)->GetInterface(recorderObject, SL_IID_ANDROIDSIMPLEBUFFERQUEUE,
                                        &recorderBufferQueue) != SL_RESULT_SUCCESS) {
        return -1;
    }

    if ((*recorderBufferQueue)->RegisterCallback(recorderBufferQueue,
                                                 &MicrophoneReader::recorderCallback, this) !=
        SL_RESULT_SUCCESS) {
        return -1;
    }

    if ((*recorderBufferQueue)->Enqueue(recorderBufferQueue, inputBuffer,
                                        FRAME_SIZE * 2 * sizeof(short)) != SL_RESULT_SUCCESS) {
        return -1;
    }

    if ((*recorderRecord)->SetRecordState(recorderRecord, SL_RECORDSTATE_RECORDING) !=
        SL_RESULT_SUCCESS) {
        return -1;
    }
