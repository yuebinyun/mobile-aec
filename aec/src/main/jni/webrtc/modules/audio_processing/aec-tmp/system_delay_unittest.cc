...
// TODO(bjornv): Add a test_abc to verify behavior if the far-end buffer is full
...
TEST_F(SystemDelayTest,
  ...
  // In this test_abc we start by establishing the device buffer size during stable
  ...
  // This test_abc assumes the reported delays are used.
  ...
  for (size_t i = 0; i < kNumSampleRates; i++) {
    ...
    // can make that assumption since we have a separate stability test_abc.
    int process_time_ms = 0;
    for (; process_time_ms < kStableConvergenceMs; process_time_ms += 10) {
      EXPECT_EQ(0,
                WebRtcAec_Process(handle_,
...
TEST_F(SystemDelayTest, CorrectDelayWhenBufferUnderrun) {
  // Here we test_abc a buffer under run scenario. If we keep on calling
...
TEST_F(SystemDelayTest, DISABLED_ON_ANDROID(CorrectDelayDuringDrift)) {
  // This drift test_abc should verify that the system delay is never exceeding the
...
TEST_F(SystemDelayTest, DISABLED_ON_ANDROID(ShouldRecoverAfterGlitch)) {
  // This glitch test_abc should verify that the system delay recovers if there is
  ...
  for (size_t i = 0; i < kNumSampleRates; i++) {
    ...
    for (int j = 0; j < 20; j++) {
      EXPECT_EQ(0, WebRtcAec_BufferFarend(handle_, far_, samples_per_frame_));
      // No need to verify system delay, since that is done in a separate test_abc.
    }
...
TEST_F(SystemDelayTest, UnaffectedWhenSpuriousDeviceBufferValues) {
  // This spurious device buffer data test_abc aims at verifying that the system
...
TEST_F(SystemDelayTest, CorrectImpactWhenTogglingDeviceBufferValues) {
  // This test_abc aims at verifying that the system delay is "unaffected" by
  // toggling values reported by the device.
  // The test_abc is constructed such that every other device buffer value is zero
  ...
  // TODO(bjornv): This test_abc will have a better impact if we verified that the
