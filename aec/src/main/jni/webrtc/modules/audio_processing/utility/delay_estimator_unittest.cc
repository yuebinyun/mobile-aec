...
TEST_F(DelayEstimatorTest, CorrectErrorReturnsOfWrapper) {
  // In this test_abc we verify correct error returns on invalid API calls.
...
TEST_F(DelayEstimatorTest, InitializedSpectrumAfterProcess) {
  // In this test_abc we verify that the mean spectra are initialized after first
  // time we call WebRtc_AddFarSpectrum() and Process() respectively. The test_abc
...
TEST_F(DelayEstimatorTest, CorrectLastDelay) {
  // In this test_abc we verify that we get the correct last delay upon valid call.
...
TEST_F(DelayEstimatorTest, CorrectErrorReturnsOfBinaryEstimatorFarend) {
  // In this test_abc we verify correct output on invalid API calls to the Binary
...
TEST_F(DelayEstimatorTest, MeanEstimatorFix) {
  // In this test_abc we verify that we update the mean value in correct direction
...
TEST_F(DelayEstimatorTest, ExactDelayEstimateMultipleNearSameSpectrum) {
  // In this test_abc we verify that we get the correct delay estimates if we shift
...
TEST_F(DelayEstimatorTest, ExactDelayEstimateMultipleNearDifferentSpectrum) {
  // In this test_abc we use the same setup as above, but we now feed the two Binary
...
TEST_F(DelayEstimatorTest, ExactDelayEstimateMultipleNearDifferentLookahead) {
  // In this test_abc we use the same setup as above, feeding the two Binary
