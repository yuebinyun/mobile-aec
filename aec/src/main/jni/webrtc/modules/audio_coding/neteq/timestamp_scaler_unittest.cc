...
// TODO(minyue): This test_abc becomes trivial since Opus does not need a timestamp
// scaler. Therefore, this test_abc may be removed in future. There is no harm to
// keep it, since it can be taken as a test_abc case for the situation of a trivial
// timestamp scaler.
...
TEST(TimestampScaler, Failures) {
  ...
  Packet* packet = NULL;
  scaler.ToInternal(packet);  // Should not crash. That's all we can test_abc.
