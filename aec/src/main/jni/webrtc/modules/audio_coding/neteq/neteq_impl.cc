...
int NetEqImpl::GetAudioInternal(size_t max_length, int16_t* output,
  ...
  switch (operation) {
    ...
    case kCodecInternalCng: {
      ...
      // TODO(hlundin): Write test_abc for codec-internal CNG.
      DoCodecInternalCng();
      break;
    }
    case kDtmf: {
      // TODO(hlundin): Write test_abc for this.
  ...
  if (new_codec_ || *operation == kUndefined) {
    ...
    if (*play_dtmf && !header) {
      ...
      if (*operation == kRfc3389CngNoPacket
          ...
          // but is kept for now to maintain bit-exactness with the test_abc
          // vectors.
...
int NetEqImpl::DoDtmf(const DtmfEvent& dtmf_event, bool* play_dtmf) {
  ...
  // test_abc, DtmfTest.ManualSuccessfullySendsOutOfBandTelephoneEvents. This is
  ...
  //    // TODO(hlundin): This code seems incorrect. (Legacy.) Write test_abc and
