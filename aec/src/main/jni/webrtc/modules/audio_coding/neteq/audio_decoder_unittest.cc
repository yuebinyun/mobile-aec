...
class AudioDecoderTest : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    // Create arrays.
    ASSERT_GT(data_length_, 0u) << "The test_abc must set data_length_ > 0";
  ...
  virtual void CompareInputOutput(size_t num_samples, int tolerance,
    ...
    for (unsigned int n = 0; n < num_samples - delay; ++n) {
      ASSERT_NEAR(input_[n], decoded_[channels_ * n + delay], tolerance) <<
          "Exit test_abc on first diff; n = " << n;
  ...
  void ReInitTest() {
    ...
    for (unsigned int n = 0; n < frame_size_; ++n) {
      ASSERT_EQ(output1[n], output2[n]) << "Exit test_abc on first diff; n = " << n;
    }
...
class AudioDecoderIlbcTest : public AudioDecoderTest {
 protected:
  ...
  // Overload the default test_abc since iLBC's function WebRtcIlbcfix_NetEqPlc does
...
// This test_abc is identical to AudioDecoderIsacSwbTest, except that it creates
...
class AudioDecoderG722StereoTest : public AudioDecoderG722Test {
 protected:
  ...
  virtual int EncodeFrame(const int16_t* input, size_t input_len_samples,
    ...
    // Encode a mono payload using the base test_abc class.
...
#ifdef WEBRTC_CODEC_CELT
// In the two following CELT tests, the low amplitude of the test_abc signal allow
