...
// This is a parameterized test_abc. The test_abc parameters are supplied through a
// TestParameters struct, which is obtained through the GetParam() method.
//
// The objective of the test_abc is to create a mono input signal and a
...
class NetEqStereoTestNoJitter : public NetEqStereoTest {
 protected:
  NetEqStereoTestNoJitter()
    ...
    // This is to avoid doing preemptive expand early in the test_abc.
...
std::list<TestParameters> GetTestParameters() {
  ...
  for (int rate_index = 0; rate_index < num_rates; ++rate_index) {
    ...
    for (int frame_size = 10; frame_size <= 60; frame_size += 10) {
      ...
      l.push_back(p);
      if (sample_rate == 8000) {
        // Add a five-channel test_abc for 8000 Hz.
        p.num_channels = 5;
        l.push_back(p);
      }
    }
...
// Pretty-printing the test_abc parameters in case of an error.
...
// Instantiate the tests. Each test_abc is instantiated using the function above,
