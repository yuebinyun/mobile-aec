...
TEST_F(NetEqDecodingTest, DISABLED_ON_ANDROID(TestBitExactness)) {
  ...
  // have a copy of the file, the test_abc will fail.
  ...
  const std::string network_stat_ref_file = webrtc::test_abc::ProjectRootPath() +
...
// TODO(hlundin): Re-enable test_abc once the statistics interface is up and again.
 ...
 protected:
  ...
  void CheckBgn(int sampling_rate_hz) {
    ...
    if (sampling_rate_hz == 8000) {
      ...
      ASSERT_TRUE(false);  // Unsupported test_abc case.
    }
    ...
    // |sampling_rate_hz|. The output may sound weird, but the test_abc is still
    // valid.
    ...
    // To be able to test_abc the fading of background noise we need at lease to
    // pull 611 frames.
    const int kFadingThreshold = 611;

    // Test several CNG-to-PLC packet for the expected behavior. The number 20
    // is arbitrary, but sufficiently large to test_abc enough number of frames.
...
// TODO(turajs) we will have a better test_abc if we have a referece NetEq, and
// when Sync packets are inserted in "test_abc" NetEq we insert all-zero payload
...
// sync packets. Also, test_abc if network packets override sync packets. That is to
