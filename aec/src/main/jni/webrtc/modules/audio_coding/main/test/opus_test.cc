...
void OpusTest::Perform() {
  ...
  // Open both mono and stereo test_abc files in 32 kHz.
  const std::string file_name_stereo =
      webrtc::test_abc::ResourcePath("audio_coding/teststereo32kHz", "pcm");
  const std::string file_name_mono =
      webrtc::test_abc::ResourcePath("audio_coding/testfile32kHz", "pcm");
  frequency_hz = 32000;
...
void OpusTest::Run(TestPackStereo* channel, int channels, int bitrate,
  ...
  // Make sure the runtime is less than 60 seconds to pass Android test_abc.
