...
// A multi-threaded test_abc for ACM. This base class is using the PCM16b 16 kHz
...
class AudioCodingModuleMtTest : public AudioCodingModuleTest {
 protected:
  ...
  virtual bool TestDone() {
    if (packet_cb_.num_calls() > kNumPackets) {
      ...
      if (pull_audio_count_ > kNumPullCalls) {
        // Both conditions for completion are met. End the test_abc.
        return true;
      }
    }
    return false;
  ...
  bool CbSendImpl() {
    SleepMs(1);
    if (HasFatalFailure()) {
      // End the test_abc early if a fatal failure (ASSERT_*) has occurred.
      test_complete_->Set();
    }
    ++send_count_;
    InsertAudio();
...
// This is a multi-threaded ACM test_abc using iSAC. The test_abc encodes audio
...
// and simplifies the test_abc code a lot.
class AcmIsacMtTest : public AudioCodingModuleMtTest {
 protected:
  ...
  // it is using the constants defined in this class (i.e., shorter test_abc run).
  virtual bool TestDone() {
    if (packet_cb_.num_calls() > kNumPackets) {
      CriticalSectionScoped lock(crit_sect_.get());
      if (pull_audio_count_ > kNumPullCalls) {
...
// This test_abc verifies bit exactness for the send-side of ACM. The test_abc setup is
// a chain of three different test_abc classes:
//
// test_abc::AcmSendTest -> AcmSenderBitExactness -> test_abc::AcmReceiveTest
//
// The receiver side is driving the test_abc by requesting new packets from
// AcmSenderBitExactness::NextPacket(). This method, in turn, asks for the
// packet from test_abc::AcmSendTest::NextPacket, which inserts audio from the
// input file until one packet is produced. (The input file loops indefinitely.)
// Before passing the packet to the receiver, this test_abc class verifies the
...
class AcmSenderBitExactness : public ::testing::Test,
 ...
 protected:
  ...
  // Sets up the test_abc::AcmSendTest object. Returns true on success, otherwise
  // false.
  bool SetUpSender() {
    const std::string input_file_name =
        webrtc::test::ResourcePath("audio_coding/testfile32kHz", "pcm");
    // Note that |audio_source_| will loop forever. The test_abc duration is set
  ...
  // Registers a send codec in the test_abc::AcmSendTest object. Returns true on
  ...
  // Runs the test_abc. SetUpSender() and RegisterSendCodec() must have been called
  ...
  void Run(const std::string& audio_checksum_ref,
    ...
    // This is where the actual test_abc is executed.
    receive_test.Run();
  ...
  // depleted (i.e., the test_abc duration is exceeded), or if an error occurred.
  // Inherited from test_abc::PacketSource.
