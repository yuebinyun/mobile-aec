...
void TestRedFec::Perform() {
  ...
  for (uint8_t n = 0; n < numEncoders; n++) {
    ...
    // Default number of channels is 2 for opus, so we change to 1 in this test_abc.
  ...
  EXPECT_TRUE(false);
  printf("G722 needs to be activated to run this test_abc\n");
  return;
#endif
  char nameG722[] = "G722";
  ...
  EXPECT_TRUE(false);
  printf("Opus needs to be activated to run this test_abc\n");
  return;
#endif
