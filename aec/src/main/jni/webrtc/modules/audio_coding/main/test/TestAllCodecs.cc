...
// Description of the test_abc:
// In this test_abc we set up a one-way communication channel from a participant
// called "a" to a participant called "b".
// a -> channel_a_to_b -> b
//
// The test_abc loops through all available mono codecs, encode at "a" sends over
...
int32_t TestPack::SendData(FrameType frame_type, uint8_t payload_type,
  ...
  // Only run mono for all test_abc cases.
...
TestAllCodecs::TestAllCodecs(int test_mode)
  ...
  // test_mode = 0 for silent test_abc (auto test_abc)
...
void TestAllCodecs::Perform() {
  ...
  if (test_mode_ != 0) {
    ...
    printf("The following codecs was not included in the test_abc:\n");
    ...
    printf("\nTo complete the test_abc, listen to the %d number of output files.\n",
           test_count_);
  }
}

// Register Codec to use in the test_abc
//
...
void TestAllCodecs::RegisterSendCodec(char side, char* codec_name,
  ...
  // fly, so we exclude this test_abc by setting |packet_size_samples_| to -1.
