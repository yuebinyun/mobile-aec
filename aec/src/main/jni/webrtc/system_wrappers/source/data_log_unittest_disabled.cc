...
void PerformLogging(const std::string& table_name) {
  ...
  ASSERT_EQ(0, DataLog::AddColumn(table_name, "test_abc", 1));
  for (int i = 0; i < 10; ++i) {
    // TODO(kjellander): Check InsertCell result when the DataLog dummy is
    // fixed.
    DataLog::InsertCell(table_name, "test_abc", static_cast<double>(i));
    ASSERT_EQ(0, DataLog::NextRow(table_name));
  }
}

// Simple test_abc to verify DataLog is still working when the GYP variable
